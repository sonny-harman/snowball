def run_escape(age_star,l_star,m_planet,r_planet,envelope_comp,efficiencies): 
      ...
      from modules import calc_escape_regime
      ...
      from planet import a_planet,core_frac,core_den,albedo
      from planet import envelope_frac,envelope_compm,envelope_species,envelope_compH
      from planet import m_star,d_star,J_mag_star
      from planet import stellar_tracks,norm_lum,do_euv_sat,do_emp_sat,do_emp_scale
      from planet import xuv_threshold,p_photo,p_xuvcofac,mode,variable_efficiency,hnu
      ...
      est_Fe,est_H2O,est_rad = bisect2(0,100,0,100,'H2O',r_planet,m_planet,planet_radius,1.E-4)
      ...
      if estimate_atmosphere:
            ...
            atm_est(r_planet,m_planet,grav_t[-1],T_skin,envelope_species,envelope_compm,e_f_t[-1]*m_p_t[-1]*m_Earth,fe_core_radius*r_Earth,vmr_t[-1],mu_t[-1],p_photo,diags,plotdir)
            if diags:
...
#                  file=saveit)
      ...
      if plots:
            ...
            plt.show()
            if save_plots:
                  fig7.savfig(plotdir+'flux_regime_boundaries_vs_time.png',dpi=200)

      return m_p_t[0], m_p_t[current_age_ind], m_p_t[-1],r_p_t[0],r_p_t[current_age_ind],r_p_t[-1],c_f_t[0],c_f_t[current_age_ind],c_f_t[-1],e_f_t[0],e_f_t[current_age_ind],e_f_t[-1],e_comp_t[0],e_comp_t[current_age_ind],e_comp_t[-1],mass_change
