...
custom_format = FuncFormatter(lambda x,_: '{0:g}'.format(x) if (0.1<=x<=10 or x==0) else '10$^{{{0:.0g}}}$'.format(math.log10(x)))
for run in data:
      single_run = data[run]
      x = single_run['age']
      ...
      for s in envelope_species:
            ...
            ax[runno].plot(x,masses[s],label=fancy_species[si],color=colors[si])#,ls=linestyles[si])
      ...
      ax[runno].set_yscale('symlog', linthresh=0.0015)
      ax[runno].text(0,0.0015,'~',fontsize=16,verticalalignment='center',horizontalalignment='center')
...
for label in (ax[-1].get_yticklabels() + ax[-1].get_xticklabels()):
      label.set_fontsize(16)
for ax_ind in ax:
      ax_ind.yaxis.set_major_formatter(custom_format)
